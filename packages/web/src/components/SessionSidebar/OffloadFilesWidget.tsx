...
export function OffloadFilesWidget({ sessionId }: OffloadFilesWidgetProps) {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/sessions/${sessionId}/offload-files`
      );
      if (!response.ok) {
        setLoadError(`获取离线文件失败 (${response.status})`);
        setFiles([]);
        return;
      }
      const data = (await response.json()) as OffloadFileListResponse;
      setFiles(data.files || []);
    } catch (error) {
  ...
  const handleOpenFile = useCallback(
    async (filename: string) => {
      ...
      try {
        const response = await fetch(
          `/api/sessions/${sessionId}/offload-files/${encodeURIComponent(
            filename
          )}`
        );
        if (!response.ok) {
          setContentError(`加载失败 (${response.status})`);
          return;
        }
        const data = (await response.json()) as OffloadFileContentResponse;
